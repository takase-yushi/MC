...
void Analyzer::storeDistributionOfMv(std::vector<CodingTreeUnit *> ctus, std::string log_path, std::vector<int> pells, std::vector<double> residuals) {
    ...
    fprintf(fp, "translation differential coding patch :%5d(%2.0f[%%])\n", translation_diff, (double)translation_diff / patch_num * 100);
    fprintf(fp, "translation merge coding patch        :%5d(%2.0f[%%])\n", translation_merge, (double)translation_merge / patch_num * 100);
    fprintf(fp, "Affine differential coding patch      :%5d(%2.0f[%%])\n", affine_diff, (double)affine_diff / patch_num * 100);
    fprintf(fp, "Affine merge coding patch             :%5d(%2.0f[%%])\n", affine_merge, (double)affine_merge / patch_num * 100);
    fprintf(fp, "Affine new merge coding patch         :%5d(%2.0f[%%])\n", affine_new_merge, (double)affine_new_merge / patch_num * 100);
