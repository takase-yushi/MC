...
/*
    v[0].x = (floor(ref_corners.p1.x / 2 + 0.5) - p0.x)/4;
    v[0].y = (floor(ref_corners.p1.y / 2 + 0.5) - p0.y)/4;
    v[1].x = (floor(ref_corners.p2.x / 2 + 0.5) - p1.x)/4;
    v[1].y = (floor(ref_corners.p2.y / 2 + 0.5) - p1.y)/4;
    v[2].x = (floor(ref_corners.p3.x / 2 + 0.5) - p2.x)/4;
    v[2].y = (floor(ref_corners.p3.y / 2 + 0.5) - p2.y)/4;
*/
/*
        ...
        for (int z = 0; z < 4; z++) {//各階層ごとに
            ...
            */
            ...
            for (int j = -expand; j < g_img.cols + expand; j++) {
                if ((g_expand[j] = (unsigned char *) std::malloc(sizeof(unsigned char) * (f_img.rows + expand * 2))) ==
                    NULL) {
                }
                g_expand[j] += expand;
            }
            ...
            if(z != 0)v_para *= 2;
/*
    v[0].x = ref_corners.p1.x / 2 - p0.x;
    v[0].y = ref_corners.p1.y / 2 - p0.y;
    v[1].x = ref_corners.p2.x / 2 - p1.x;
    v[1].y = ref_corners.p2.y / 2 - p1.y;
...
*/
            ...
            for (int q = 0; q < 11; q++) {//ガウスニュートン法を11回繰り返す
                ...
                /*if(p0.x <= 0 || p0.x >= f_img.cols -1 || p0.y <= 0 || p0.y >= f_img.rows -1 ||
                   p1.x <= 0 || p1.x >= f_img.cols -1 || p1.y <= 0 || p1.y >= f_img.rows -1 ||
                ...
                for (int m = 0; m < (int) in_triangle.size(); m++) {//パッチ内の画素ごとに
                    ...
                    //std::cout << "X_later = " << X_later << std::endl;
                    ...
                    //g1 = M(g_img, x0_delta, y0_delta) * (1 - d_x_delta) * (1 - d_y_delta) +
            ...
            if(z == 3) {
                ...
                for (int j = 0; j < 3; j++) {
                    ...
                    //mv[j].x = floor(tmp_x + 0.5);
                    //mv[j].y = floor(tmp_y + 0.5);
                    //std::cout << "pp[" << j << "] = " << pp[j].x << ", " << pp[j].y << std::endl;
                }
            }
    ...
    if(z == 3){
        if(*flag == true){
            ...
            for (int i = 0; i < (int) in_triangle.size(); i++) {
                ...
                // std::cout << "alpha = " << alpha << "beta = " << beta << std::endl;
                // std::cout << "X_later = " << X_later.x << ", " << X_later.y << std::endl;
                if (X_later.x >= g_img.cols - 1) {
                    X_later.x = g_img.cols - 1.001;
                }
                if (X_later.y >= g_img.rows - 1) {
...
*/
        ...
        if(z == 5){
            ...
            std::cout << "block_mv = " << block_mv.x << ", " << block_mv.y << std::endl;
            tri_list << ", block ," << block_mv.x << ", " << block_mv.y << "," << block_error << ",";
        }
        else {
            for (int q = 0; q < 11; q++) {
                //while(Residual_Error >= th) {
                ...
                S_[5] = 0.5*(b.x + d.x);
                tri_S0 = fabs(det);
                tri_list << "tri_S0_prev, " << tri_S0 << ", ";
                /*
                if (z == 1 && tri_S0 < 1024 && q == 0) {
                    for (int i = 0; i < 3; i++) {
                        v[i].x = 0;
                        v[i].y = 0;
                    }
                ...
                for (int m = 0; m < (int) in_triangle.size(); m++) {
                    ...
                    alpha = (X.x * b.y - X.y * b.x) / det;
                    beta = (a.x * X.y - a.y * X.x) / det;
                    //if(t >= 32 && t <= 33)  std::cout << "check 4" << std::endl;
                    //std::cout << "alpha = " << alpha << " beta = " << beta << std::endl;
                    X.x += p0.x;
                    X.y += p0.y;
                    x0 = (int) floor(X.x);
                    ...
                    d_x = X.x - x0;
                    d_y = X.y - y0;
                    //if(t >= 32 && t <= 33) std::cout << "check 5" << std::endl;
                    //std::cout << "X = (" << X.x <<" , " << X.y << ")" << std::endl;
                    ek_tmp = 0;
                    for (int i = 0; i < 6; i++) {
                        //if(t >= 32 && t <= 33)     std::cout << "check 6" << std::endl;
                        pp[0].x = p0.x + v[0].x;
                        pp[0].y = p0.y + v[0].y;
                        pp[1].x = p1.x + v[1].x;
                        pp[1].y = p1.y + v[1].y;
                        pp[2].x = p2.x + v[2].x;
                        pp[2].y = p2.y + v[2].y;
                        //if(t >= 32 && t <= 33)   std::cout << "check 7" << std::endl;
                        triangle_later.clear();
                        triangle_later.emplace_back(pp[0]);
                        triangle_later.emplace_back(pp[1]);
                        triangle_later.emplace_back(pp[2]);
                        //if(t >= 32 && t <= 33)  std::cout << "check 8" << std::endl;
                        pp_para[0] = p0 + v_para;
                        pp_para[1] = p1 + v_para;
                        pp_para[2] = p2 + v_para;
                        //if(t >= 32 && t <= 33)  std::cout << "check 9" << std::endl;
                        triangle_later_para.clear();
                        triangle_later_para.emplace_back(pp_para[0]);
                        triangle_later_para.emplace_back(pp_para[1]);
                        triangle_later_para.emplace_back(pp_para[2]);
                       // if(t >= 32 && t <= 33)   std::cout << "check 10" << std::endl;
                        switch (i) {
                            case 0:
                                pp[0].x += delta;
                                delta_x = 1 - alpha - beta;
                       ...
                       // if(t >= 32 && t <= 33)     std::cout << "check 13" << std::endl;
                       ...
                       // if(t >= 32 && t <= 33)   std::cout << "check 14" << std::endl;
                        ...
                        //if(t >= 32 && t <= 33)     std::cout << "check 15" << std::endl;
                        ...
                        g_y = g_expand[(int) X_later.x][(int) X_later.y + 1] -
                              g_expand[(int) X_later.x][(int) X_later.y];
                        //if(t >= 32 && t <= 33)     std::cout << "check 16" << std::endl;
                    ...
                    X_later.y = alpha * a_later.y + beta * b_later.y + triangle_later[0].y;
                    X_later_para = alpha * a_later_para + beta * b_later_para + triangle_later_para[0];
                    //if(t >= 32 && t <= 33) std::cout << "check 23" << std::endl;
                    /*
            if (X_later.x >= g_img.cols - 1) {
                X_later.x = g_img.cols - 1.001;
            }
            ...
            if(X_later.y < 0){
                X_later.y = 0;
                    ...
                    g1 = g_expand[x0_later][y0_later] * (1 - d_x_later) * (1 - d_y_later) +
                         ...
                         g_expand[x0_later][y0_later + 1] * (1 - d_x_later) * d_y_later +
                         g_expand[x0_later + 1][y0_later + 1] * d_x_later * d_y_later;
                    //if(t >= 32 && t <= 33)  std::cout << "check 27" << std::endl;
                    ...
                    MSE += (f - g) * (f - g);
                    MSE_para += (f - g_para) * (f - g_para);
                    //if(t >= 32 && t <= 33)  std::cout << "check 35" << std::endl;
                ...
                tri_list << "tri_S1_later, " << tri_S1 << ", tri_per, " << tri_per << ", %, ";
                for (int t = 0; t < dim; t++) {
                    delta_ek[t] *= -2;
                ...
                //  std::cout << "current_error = " << current_error << std::endl;
                // std::cout << "Residual_Error = " << Residual_Error << std::endl;
                tri_list << "PSNR[" << q << "], " << PSNR << ", PSNR_para[" << q << "], " << PSNR_para << std::endl;
            }
...
/*
            ...
            */
            ...
            if (z == 3) {
                if(*flag == true){
                    if(MSE_para_min > erase_th_global) {
                        R(color, (int) X.x, (int) X.y) = (unsigned char) 255;
                        G(color, (int) X.x, (int) X.y) = (unsigned char) 0;
                        B(color, (int) X.x, (int) X.y) = (unsigned char) 0;
                    }
                    else {
                        R(color, (int) X.x, (int) X.y) = (unsigned char) 0;
                        G(color, (int) X.x, (int) X.y) = (unsigned char) 255;
                        B(color, (int) X.x, (int) X.y) = (unsigned char) 0;
                    }
                }
                else{
                    if(MSE_min > erase_th_global){
                        R(color, (int) X.x, (int) X.y) = (unsigned char) 255;
                        G(color, (int) X.x, (int) X.y) = (unsigned char) 0;
                        B(color, (int) X.x, (int) X.y) = (unsigned char) 0;
                    }else{
                ...
                /*
                R(color, (int) X.x, (int) X.y) = (unsigned char) (255 * (1 - alpha_color));
                G(color, (int) X.x, (int) X.y) = (unsigned char) (255 * alpha_color);
                B(color, (int) X.x, (int) X.y) = (unsigned char) 0;
                 */
            }
            //std::cout << "X_later = " << X_later.x << ", " << X_later.y << std::endl;
            // std::cout << i << "/ " << in_triangle.size() << std::endl;
        }
        /*
        if(z == 3) {
            for (int i = 0; i < (int) in_triangle.size(); i++) {
    ...
    if(PSNR_max < 50 && S > 5000 && add_count < 100 && add_corner_flag) {
        ...
        if(p0.x < grav.x)p0.x += 8;
        else p0.x -= 8;
        if(p0.y < grav.y)p0.y += 8;
        else p0.y -= 8;
        if(p1.x < grav.x)p1.x += 8;
        else p1.x -= 8;
        if(p1.y < grav.y)p1.y += 8;
        else p1.y -= 8;
        if(p2.x < grav.x)p2.x += 8;
        else p2.x -= 8;
        if(p2.y < grav.y)p2.y += 8;
        else p2.y -= 8;

        const int POINT_MAX = 10;
        cv::Mat mask = cv::Mat::zeros(residual.rows, residual.cols, CV_8UC1);
        Point3Vec triangleVec = Point3Vec(p0, p1, p2);
        cv::Point2f xp;
        ...
        for (int j = 0; j < residual.rows; j++) {
            for (int i = 0; i < residual.cols; i++) {
                ...
                if (isInTriangle(triangleVec, xp) == 1) {
                    ...
                    grav_residual.x += xp.x*residual.at<unsigned char>(xp.y,xp.x);
                    grav_residual.y += xp.y*residual.at<unsigned char>(xp.y,xp.x);
                    residual_sum += residual.at<unsigned char>(xp.y,xp.x);
                } else {
                    mask.at<unsigned char>(j, i) = 0;
                }
            }
        }
        grav_residual /= residual_sum;
        grav_residual.x = (int)(grav_residual.x + 0.5);
        grav_residual.y = (int)(grav_residual.y + 0.5);
        ...
        for(const cv::Point2f p : add_corners){
            std::cout << "add_corner = " << p << std::endl;
        }
    }
    if(*flag == true){
