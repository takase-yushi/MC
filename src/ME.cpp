...
std::tuple<std::vector<cv::Point2f>, cv::Point2f, double, double, int> Square_GaussNewton(std::vector<std::vector<cv::Mat>> ref_images, std::vector<std::vector<cv::Mat>> target_images, std::vector<std::vector<std::vector<unsigned char *>>> expand_image, Point4Vec target_corners, int square_index, CodingTreeUnit *ctu, cv::Point2f init_vector, unsigned char *ref_hevc){
    ...
    for(int filter_num = 0 ; filter_num < static_cast<int>(ref_images.size()) ; filter_num++){
        ...
        for(int step = 3 ; step < static_cast<int>(ref_images[filter_num].size()) ; step++){
            ...
            if (target_corners.p4.x == target_images[0][3].cols - 1) p3.x = target_images[0][step].cols - 1;
            if (target_corners.p4.y == target_images[0][3].rows - 1) p3.y = target_images[0][step].rows - 1;
            ...
            while(true){
                ...
                for(const auto& pixel : pixels_in_square) {
                    ...
                    // 移動後の頂点を計算し格納
                    std::vector<cv::Point2f> square_later_translation(3);
                    square_later_translation[0] = p0 + tmp_mv_translation;
                    square_later_translation[1] = p1 + tmp_mv_translation;
                    square_later_translation[2] = p2 + tmp_mv_translation;

                    cv::Point2f a_later_translation = square_later_translation[2] - square_later_translation[0];
                    cv::Point2f b_later_translation = square_later_translation[1] - square_later_translation[0];
                    cv::Point2f X_later_translation = alpha * a_later_translation + beta * b_later_translation + square_later_translation[0];
                ...
                if(translation_update_flag && prev_SSE_translation > SSE_translation) {
                    for (int k = 0; k < 2; k++) {
                        if (k % 2 == 0) {
                            ...
                            if ((-scaled_spread <= scaled_coordinates[0].x + translated_x) && (target_images[0][step].cols - 1 + scaled_spread >= scaled_coordinates[0].x + translated_x) &&
                                (-scaled_spread <= scaled_coordinates[1].x + translated_x) && (target_images[0][step].cols - 1 + scaled_spread >= scaled_coordinates[1].x + translated_x) &&
                                (-scaled_spread <= scaled_coordinates[2].x + translated_x) && (target_images[0][step].cols - 1 + scaled_spread >= scaled_coordinates[2].x + translated_x)) {
                                tmp_mv_translation.x = translated_x;
                            }
                        } else {
                            double translated_y = tmp_mv_translation.y + delta_v;
                            if ((-scaled_spread <= scaled_coordinates[0].y + translated_y) && (target_images[0][step].rows - 1 + scaled_spread >= scaled_coordinates[0].y + translated_y) &&
                                (-scaled_spread <= scaled_coordinates[1].y + translated_y) && (target_images[0][step].rows - 1 + scaled_spread >= scaled_coordinates[1].y + translated_y) &&
                                (-scaled_spread <= scaled_coordinates[2].y + translated_y) && (target_images[0][step].rows - 1 + scaled_spread >= scaled_coordinates[2].y + translated_y)) {
...
//            }
            ...
            tmp_mv_translation = prev_mv_translation;
            double Error_translation = prev_SSE_translation;
    ...
    for(int filter_num = 0 ; filter_num < static_cast<int>(ref_images.size()) ; filter_num++){
        ...
        for(int step = 3 ; step < static_cast<int>(ref_images[filter_num].size()) ; step++){
            ...
            if(step != 0) {
                ...
                // ワーピング
                double magnification = 1.0; //1.0以上になったら右下の頂点がはみ出す場合も考慮しないといけない
            ...
            while(true){
                ...
                for(auto pixel : pixels_in_square) {
                    ...
                    // 移動後の頂点を計算し格納
                    std::vector<cv::Point2f> square_later_warping(3);
                    square_later_warping[0] = p0 + tmp_mv_warping[0];
                    square_later_warping[1] = p1 + tmp_mv_warping[1];
                    square_later_warping[2] = p2 + tmp_mv_warping[2];

                    cv::Point2f a_later_warping  =  square_later_warping[2] -  square_later_warping[0];
                    cv::Point2f b_later_warping  =  square_later_warping[1] -  square_later_warping[0];
                    cv::Point2f  X_later_warping  = alpha *  a_later_warping + beta *  b_later_warping +  square_later_warping[0];
            ...
            tmp_mv_warping = prev_mv_warping;//一番良い動きベクトルを採用
            double Error_warping = prev_SSE_warping;
