...
/**
 * @fn std::pair<std::vector<cv::Point2f>, cv::Point2f> GaussNewtonForSquare(cv::Mat ref_image, cv::Mat target_mage, cv::Mat gauss_ref_image, Point3Vec target_corners)
 * @brief ガウス・ニュートン法を行い、動きベクトル・予測残差・面積を返す
 * @param ref_images
 * @param target_images
 * @param expand_image
...
std::tuple<std::vector<cv::Point2f>, cv::Point2f, double, double, int> GaussNewtonForSquare(std::vector<std::vector<cv::Mat>> ref_images, std::vector<std::vector<cv::Mat>> target_images, std::vector<std::vector<std::vector<unsigned char *>>> expand_image, Point3Vec target_corners, cv::Point2f init_vector, unsigned char *ref_hevc){
    ...
    cv::Point2f pp1 = target_corners.p1;
    cv::Point2f pp2 = target_corners.p2;
    cv::Point2f pp3 = target_corners.p3;
    cv::Point2f pp4 = target_corners.p3;
    pp4.x = pp2.x;

    pixels_in_triangle = getPixelsInSquare(pp1, pp2, pp3, pp4);
    ...
    for(int filter_num = 0 ; filter_num < static_cast<int>(ref_images.size()) ; filter_num++){
        ...
        for(int step = 3 ; step < static_cast<int>(ref_images[filter_num].size()) ; step++){
            ...
            while(true){
                ...
                for (int k = 0; k < 6; k++) {
                    if (k % 2 == 0) {
                        //動きベクトルを更新(画像の外に出ないように)
                        double after_coordinate_x = scaled_coordinates[(int) (k / 2)].x + tmp_mv_warping[(int) (k / 2)].x + delta_uv_warping.at<double>(k, 0);

                        if ((-scaled_spread <= tmp_pp4.x) && (tmp_pp4.x <= target_images[0][step].cols - 1 + scaled_spread) &&
                                (-scaled_spread <= after_coordinate_x) && after_coordinate_x <= (target_images[0][step].cols - 1 + scaled_spread)) {
                            tmp_mv_warping[(int) (k / 2)].x = tmp_mv_warping[(int) (k / 2)].x + delta_uv_warping.at<double>(k, 0);
                        }
                    } else {
                        double after_coordinate_y = scaled_coordinates[(int) (k / 2)].y + tmp_mv_warping[(int) (k / 2)].y + delta_uv_warping.at<double>(k, 0);

                        if ((-scaled_spread <= tmp_pp4.y) && (tmp_pp4.y <= target_images[0][step].rows - 1 + scaled_spread) &&
                                (-scaled_spread <= after_coordinate_y) && after_coordinate_y <= (target_images[0][step].rows - 1 + scaled_spread)) {
